...
2. SPA is not SEO friendly as search engine crawler gets an almost empty page from the server with no data.
...
The intriguing thing is that you are not even required to write the beck-end in a traditional way. Instead of creating a back-end API, you can write code that can fetch data directly from the database during Static Site Generation. For SSR you should provide the following methods for your page component:
...
Images usually take the biggest part of your page size and have the biggest influence on our page loading time.
...
It’s possible to optimize image loading by the means of lazy loading. It is a technique to load images only when they are needed instead of loading them beforehand. Let’s say you have 100 images on the page. Probably only some of them are shown until a user scrolls down the page and you can postpone loading images that are currently not visible on the viewport and load them as a user scrolls the page.
...
Let me elaborate on the topic of React vs Preact to convince you that you should give Preact a try if you want to strip down your web application to the minimal size.
...
A service worker is a proxy that sits between the web app, the browser, and the network (when available). It allows to intercept network requests and take appropriate action based on whether the network is available such as caching or reading from the cache. With a service worker, you can cache static assets and API responses and create a Progressive Web App (PWA) that can be installed on mobile phones and used offline.
...
Improving web app performance with Import Cost plugin
...
After the run you will get the following screen with results:
...
If you need the best performance and lowest load time, strip down your site to the bare minimum, remove redundant packages such as Bootstrap and Material, MomentJs, etc. Compress your images and use modern image formats such as WebP and Avif. Pre-generate your pages with SSG.
